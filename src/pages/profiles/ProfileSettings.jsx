...
const schema = yup.object({
  ...
  username: yup.string().required("Username kamu masih kosong"),
...
export default function ProfileSettings() {
  ...
  const { setUserProfile } = useUserDetails()
  ...
  async function onSubmit() {
    try {
      ...
      const id = session.id
      const formated = new URLSearchParams(formData)
      const result = await fetch(`${API}/profiles/${id}`, {
        method: "PATCH",
        headers: {
